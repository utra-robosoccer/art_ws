...
LMS1xx::LMS1xx() :
    connected(false), globalBufSize(0) {
}
...
bool LMS1xx::getData(scanData& data) {
    ...
    // Initialize the buf with any leftovers from last time
    memcpy(buf, globalBuf, globalBufSize);
    ...
    // Check to see if we already have enough in the buf for a complete packet
    for (int i = 0; i < len && !found_packet; i++)
    {
        if (buf[i] == endOfMessage)
        {
            // Okay! Store any leftovers (beyond the endOfMessage) in the
            // globalBuf, for next call to getData to use
            globalBufSize = (len - i - 1);
            memcpy(globalBuf, buf + i + 1, globalBufSize);

            // adjust the len to be the len of the packet
            len = i + 1;

            // Flag that we found a packet
            found_packet = true;
        }
    }

    // If we don't have a full packet yet, continually read new data until we do
    while (!found_packet)
    {
        FD_ZERO(&rfds);
        ...
        if (retval) {
            ...
            // Yay, new data available! Read as much as is available (without overflowing buf)
            int original_len = len;
            len += read(sockDesc, buf + len, 20000 - len);

            // Check for endOfMessage
            for (int i = original_len; i < len && !found_packet; i++)
            {
                if (buf[i] == endOfMessage)
                {
                    // Okay! Store any leftovers (beyond the endOfMessage) in the
        ...
        {
            // Oops, we timed out! Nothing to do!
        }
    }
